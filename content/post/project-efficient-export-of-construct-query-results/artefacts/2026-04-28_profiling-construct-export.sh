...
# Configuration
SERVER_BIN="/home/userNoPriv/code/qlever/qlever-code/build-08be2975-Profiling/qlever-server"
SERVER_PORT=7001
...
start_server() {
  ...
  while lsof -ti:"$SERVER_PORT" >/dev/null 2>&1; do
    ...
    ((waited++))
    if ((waited > 50)); then
      echo "ERROR: port $SERVER_PORT still in use after 10 s" >&2
      exit 1
    fi
  done
...
stop_server() {
  ...
  while lsof -ti:"$SERVER_PORT" >/dev/null 2>&1; do
    ...
    ((waited++))
    if ((waited > 50)); then
      echo "ERROR: port $SERVER_PORT not released after 10 s" >&2
      exit 1
    fi
  done
}
